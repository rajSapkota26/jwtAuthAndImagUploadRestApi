package com.myshop.imageHandle;
...
@RestController
@CrossOrigin("*")
public class ImageUploadsController {
	...
	public MyImage uploadImage(@RequestBody MultipartFile file) {
		...
		try {			
			...
			if(uploadFile) {
				...
				imag=new MyImage();		
				imag.setDownloadLink((ServletUriComponentsBuilder.fromCurrentContextPath().path("/images/").path(imageUrl).toUriString()));
			return 	imag;			
			}			
		} catch (Exception e) {
			e.printStackTrace();			
